...
export function getFruitNames(fruits: IFruit[]): string[] {
  return fruits.map((fruit) => fruit.fruitName);
}

// Fungsi untuk mengelompokkan buah berdasarkan tipe
export function groupFruitsByType(fruits: IFruit[]): {
  [key: string]: string[];
} {
...
export function getFruitTypesAndWadah(fruits: IFruit[]): {
  jumlahWadah: number;
  wadah: { [key: string]: string[] };
} {
...
export function getTotalStockByType(fruits: IFruit[]): {
  [key: string]: number;
} {
...
export function getComments(fruits: IFruit[]): string[] {
  ...
  if (fruitNames.length !== uniqueFruitNames.size) {
    comments.push(
      "Perhatian: Ada duplikasi nama buah dengan huruf kapital yang berbeda (misalnya, 'Apel' dan 'apel')."
    );
  }
  ...
  if (fruitTypes.size > 1) {
    comments.push(
      "Buah telah dipisahkan menjadi beberapa wadah berdasarkan tipe: 'IMPORT' dan 'LOCAL'."
    );
  }
  ...
  for (const id in fruitIdCounts) {
    if (fruitIdCounts.hasOwnProperty(id)) {
      ...
      if (count > 1) {
        comments.push(
          `Perhatian: Ada beberapa buah dengan 'fruitId' yang sama (ID: ${id}).`
        );
      }
    }
  }
  ...
  if (highStockFruit.length > 0) {
    comments.push(
      "Perhatian: Beberapa buah memiliki stok yang sangat tinggi, seperti 'Salak' dengan stok 150."
    );
  }
